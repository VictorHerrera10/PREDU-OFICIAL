...
type CareerArea = 'Ingenierías' | 'Sociales' | 'Biomédicas';
type RiasecProfile = 'realista' | 'investigador' | 'artistico' | 'social' | 'emprendedor' | 'convencional';
...
const CAREER_MAP: Record<string, CareerArea> = {
    ...
    'ciencias de la salud': 'Biomédicas',
    'medicina': 'Biomédicas',
};

const MATRIX: Partial<Record<CareerArea, Partial<Record<RiasecProfile, Recommendation>>>> = {
    Ingenierías: {
        realista: {
...
export function getRecommendation(academicResult: string, psychologicalResult: string): Recommendation {
    ...
    const normalizedPsychological = normalizeString(psychologicalResult) as RiasecProfile;

    const academicArea = CAREER_MAP[normalizedAcademic] || null;
    
    if (academicArea && normalizedPsychological) {
        const specificRecommendation = MATRIX[academicArea]?.[normalizedPsychological];
